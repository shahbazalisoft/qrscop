                ...
                <div class="d-flex flex-wrap justify-content-between w-100 row-gap-1">
                    <ul class="nav nav-tabs nav--tabs border-0 gap-2">
                        <li class="nav-item mr-2 mr-md-4">
                            <a href="{{route('admin.settings.email.index')}}" class="nav-link pb-2 px-0 pb-sm-3">
                                <img src="{{asset('/public/assets/admin/img/mail-config.png')}}" alt="">
                                <span>{{translate('Mail Config')}}</span>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="{{route('admin.settings.email.test')}}" class="nav-link pb-2 px-0 pb-sm-3 active">
                    ...
                    <div class="py-1">
                        <div class="text--primary-2 d-flex flex-wrap align-items-center" type="button">
                            <a href="{{ redirect()->back()->getTargetUrl() }}" class="btn btn--reset">
            ...
            if (ValidateEmail($('#test-email').val())) {
                Swal.fire({
                    ...
                    if (result.value) {
                        ...
                        $.ajax({
                            url: "{{ route('admin.settings.email.send') }}",
                            method: 'GET',
                            data: {
