    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','contact_us_mail_status_admin')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        <div class="maintenancemode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Receive_Mail_On_Contact_Us?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_someone_submits_a_contact_us_form,_admin_will_receive_an_automated_email.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-title-on="{{translate('Want_to_enable_Contact_Us_mail?')}}"
                                       data-title-off="{{translate('Want_to_disable_Contact_Us_mail?')}}"
                                       data-text-on="<p>{{translate('If_enabled,_the_admin_will_get_an_automated_email_when_someone_submits_contact_us.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_the_admin_will_not_get_an_automated_email_when_someone_submits_contact_us.')}}</p>"
                        ...
                        <form action="{{route('admin.settings.email.email-status',['admin','contact-us',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','admin')->where('email_type', 'contact_us')->first())
                @php($template= $template ?? $data?->email_template ?? 2)
                <form action="{{ route('admin.settings.email.admin_template', ['contact-us']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','admin')->where('email_type', 'contact_us')->first())
                                        ...
                                        @if ($language)
                                            <div class="__bg-F8F9FC-card default-form lang_form" id="default-form">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="title[]" value="{{ $data?->getRawOriginal('title') }}" data-id="mail-title" placeholder="{{translate('New Contact Us Message')}}" class="form-control">
                                                </div>
                                            ...
                                            if($data && count($data['translations'])){
                                                ...
                                                <div class="__bg-F8F9FC-card d-none lang_form" id="{{$lang}}-form">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" name="title[]" placeholder="{{translate('New Contact Us Message')}}" class="form-control" value="{{$translate[$lang]['title']??''}}">
                                                    </div>
                                                    <div class="form-group mb-0">
                                                        <label class="form-label">
                                        ...
                                        @else
                                            <div class="__bg-F8F9FC-card default-form">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="title[]" placeholder="{{translate('New Contact Us Message')}}" class="form-control">
                                                </div>
                                                <div class="form-group mb-0">
                                                    <label class="form-label">
                                        ...
                                        <div class="__bg-F8F9FC-card">
                                                @if ($language)
                                                        <div class="form-group lang_form default-form">
                                                            ...
                                                            <input type="text" data-id="mail-footer" name="footer_text[]"  placeholder="{{ translate('Please_contact_us_for_any_queries_we_are_always_happy_to_help') }}" class="form-control" value="{{ $data?->getRawOriginal('footer_text') }}">
                                                        </div>
                                                    @foreach(json_decode($language) as $lang)
                                                    <?php
                                                    if($data && count($data['translations'])){
                                                        ...
                                                        <div class="form-group d-none lang_form" id="{{$lang}}-form2">
                                                            ...
                                                            <input type="text" name="footer_text[]"  placeholder="{{ translate('Please_contact_us_for_any_queries_we_are_always_happy_to_help') }}" class="form-control" value="{{ $translate[$lang]['footer_text']??'' }}">
                                                        </div>
                                                    @endforeach
                                                @else
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" placeholder="{{ translate('Please_contact_us_for_any_queries_we_are_always_happy_to_help') }}" class="form-control" name="footer_text[]" value="">
                                                </div>
                                                @endif
