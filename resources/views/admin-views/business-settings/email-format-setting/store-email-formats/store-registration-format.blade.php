            ...
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    ...
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_Store_registers_from_the_Customer_app_or_Website,_Admin_Landing_Page_or_Store_app,_they_will_get_a_confirmation_email.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,_the_vendor_will_get_an_automated_email_when_a_vendor_registers.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_the_vendor_will_not_get_an_automated_email_when_a_vendor_registers.')}}</p>"
                        ...
                        <form action="{{route('admin.settings.email.email-status',['store','registration',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                        {{-- {{route('admin.business-settings.email-status',['store','registration',$mail_status == '1'?0:1])}} --}}
                    </div>
                </div>
                
                @php($data=\App\Models\EmailTemplate::where('type','store')->where('email_type', 'store_registration')->first())
                @php($template=$template?$template:($data?$data->email_template:1))
                <form action="{{ route('admin.settings.email.vendor_template', ['store-registration']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','store')->where('email_type', 'store_registration')->first())
