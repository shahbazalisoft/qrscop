    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','contact_us_mail_status_store')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        <div class="maintenance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send Mail on Contact Us?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_someone_submits_a_contact_us_form,_store_will_receive_an_automated_email.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,_stores_will_get_a_Contact_Us_email_when_someone_submits_contact_us.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_stores_will_not_get_a_Contact_Us_email_when_someone_submits_contact_us.')}}</p>"
                        ...
                        <form action="{{route('admin.settings.email.email-status',['store','contact-us',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','store')->where('email_type', 'contact-us')->first())
                @php($template= $template ?? $data?->email_template ?? 2)
                <form action="{{ route('admin.settings.email.vendor_template', ['contact-us']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','store')->where('email_type', 'contact-us')->first())
