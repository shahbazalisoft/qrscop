...
<div class="content container-fluid">
    ...
    <div class="js-nav-scroller hs-nav-scroller-horizontal mb-4">
        <ul class="nav nav-tabs border-0 nav--tabs nav--pills">
            ...
            <li class="nav-item">
                <a href="{{ route('admin.settings.subscription.subscriptionackage.transaction',$subscriptionackage->id) }}" class="nav-link">{{ translate('Transactions') }}</a>
            </li>
        </ul>
    </div>
    ...
    <div class="card __billing-subscription mb-3">
        <div class="card-header border-0 align-items-center">
            ...
            <div class="d-flex gap-2 align-items-center justify-content-center">
                <label class="toggle-switch toggle-switch-sm"> {{ translate('Status') }}:&nbsp;
                    <input type="checkbox"  data-package_id="{{$subscriptionackage->id}}" data-package_name="{{$subscriptionackage->package_name}}" data-url="{{route('admin.settings.subscription.subscriptionackage.status',[$subscriptionackage->id,$subscriptionackage->status?0:1])}}" class="toggle-switch-input {{$subscriptionackage->status?'status_change_alert':'status_change_alert_reenable'}}" {{$subscriptionackage->status?'checked':''}}>
                    <span class="toggle-switch-label">
                        <span class="toggle-switch-indicator"></span>
                    </span>
                </label>
                <div>
                    <a class="btn btn--primary py-2" href="{{ route('admin.settings.subscription.subscriptionackage.edit',$subscriptionackage->id) }}" title="Edit Package"><i class="tio-edit"> </i> {{ translate('Edit') }}</a>
                </div>
            </div>
        </div>
        ...
        <div class="card-body pt-0">
            <div class="__bg-F8F9FC-card __plan-details">
                ...
                <div class="check--grid-wrapper mt-3 max-w-850px">
                    ...
                    <div>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            @else
                            <span class="form-check-label text-dark"> {{ $subscriptionackage->max_order }} {{ translate('Orders') }}</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            @endif
                            <span class="form-check-label text-dark">{{ translate('QR Menu') }}</span>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                            @endif
                            <span class="form-check-label text-dark">{{ translate('Online Ordering') }}</span>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="d-flex align-items-center gap-2">
                            ...
                            @else
                            <span class="form-check-label text-dark"> {{ $subscriptionackage->max_product }} {{ translate('Menu Items') }}</span>
                            @endif
                        </div>
                    </div>
                ...
                @if($subscriptionackage->menuTemplates && $subscriptionackage->menuTemplates->count() > 0)
                <div class="mt-3">
                    <h6 class="text--title mb-2">{{ translate('Included_Menu_Templates') }}</h6>
                    <div class="d-flex flex-wrap gap-2">
                        @foreach($subscriptionackage->menuTemplates as $template)
...
<div class="modal fade" id="shift_package">
    <div class="modal-dialog modal-dialog-centered status-warning-modal">
        <div class="modal-content">
            ...
            <form action="{{ route('admin.settings.subscription.subscriptionackage.switchPlan') }}" method="post">
                @csrf
...
"use strict";
    ...
    $(document).on('click', '.order_stats_update', function () {
        $.get({
            url: '{{route('admin.settings.subscription.subscriptionackage.overView',$subscriptionackage->id)}}',
            dataType: 'json',
            data: {
                type: $(this).val(),
            },
