...
@section('content')
    ...
    <form action="{{ route('admin.settings.subscription.subscriptionackage.update',$subscriptionackage->id) }}" method="post">
        @csrf
        @method('put')
            ...
            @if(isset($menuTemplates) && $menuTemplates->count() > 0)
            <div class="card mb-20">
                <div class="card-header">
                    <div class="w-100 d-flex flex-wrap align-items-start gap-2">
                        <img src="{{asset('/public/assets/admin/img/material-symbols_featured-play-list-2.png')}}" alt="img" class="mt-1">
                        <div class="w-0 flex-grow">
                            <h5 class="text--title card-title d-flex gap-3 flex-wrap mb-1">
                                <div>
                                    {{ translate('Select_Menu_Templates') }}
                                </div>
                                <label class="form-group form-check form--check">
                                    <input type="checkbox" class="form-check-input" id="select-all-templates">
                                    <span class="form-check-label text-dark font-regular text-14">{{ translate('Select_All') }}</span>
                                </label>
                            </h5>
                            <div class="fz-12px">{{ translate('Select_menu_templates_to_include_in_this_package') }}</div>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="row g-3">
                        @php
                            $selectedTemplateIds = $subscriptionackage->menuTemplates->pluck('id')->toArray();
                        @endphp
                        @foreach($menuTemplates as $template)
                        <div class="col-lg-3 col-md-4 col-sm-6">
                            <label class="form-check form--check d-flex align-items-center gap-2 p-3 border rounded cursor-pointer">
                                <input type="checkbox" class="form-check-input menu-template-check" name="menu_template_ids[]" value="{{ $template->id }}" {{ in_array($template->id, $selectedTemplateIds) ? 'checked' : '' }}>
                                <span class="form-check-label text-dark">{{ $template->title }}</span>
                            </label>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="card-header">
                    <div class="w-100 d-flex flex-wrap align-items-start gap-2">
                        ...
                        <div class="w-0 flex-grow">
                            ...
                            <div class="fz-12px">{{ translate('Set maximum order & menu item limit for this package') }}</div>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="d-flex flex-wrap gap-3">
                        <div class="__bg-F8F9FC-card p-0">
                            <div class="card-body">
                                <div class="limit-item-card">
                                    <div class="form-group mb-0">
                                        <label class="form-label text-capitalize">{{ translate('Maximum Order Limit') }}</label>
                        ...
                        <div class="__bg-F8F9FC-card p-0">
                            <div class="card-body">
                                <div class="limit-item-card">
                                    <div class="form-group mb-0">
                                        <label class="form-label text-capitalize">{{ translate('Maximum Menu Item Limit') }}</label>
                                        <div class="d-flex flex-wrap items-center gap-2">
                                            <div class="resturant-type-group p-0">
                                                <label class="form-check form--check mr-2 mr-md-4">
...
"use strict";
    ...
    $('#select-all-templates').on('change', function(){
        $('.menu-template-check').prop('checked', $(this).is(':checked'));
    });
    $('.menu-template-check').on('change', function(){
        $('#select-all-templates').prop('checked', $('.menu-template-check').length == $('.menu-template-check:checked').length);
    }).trigger('change');
