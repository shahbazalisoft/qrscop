    ...
    <div class="content container-fluid">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal mb-4">
            <ul class="nav nav-tabs border-0 nav--tabs nav--pills">
                ...
                <li class="nav-item">
                    <a href="{{ route('admin.settings.subscription.subscriptionackage.subscriberTransactions',$store->id) }}" class="nav-link">{{ translate('Transactions') }}</a>
                </li>
                <li class="nav-item">
                    <a href="{{ route('admin.settings.subscription.subscriptionackage.subscriberWalletTransactions',$store->id) }}" class="nav-link">{{ translate('Subscription_Refunds') }}</a>
                </li>
            </ul>
        </div>
                    ...
                    <div class="check--grid-wrapper mt-3 max-w-850px">
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="form-check-label text-dark">{{ addon_published_status('Rental') ? translate('messages.unlimited_trips') : translate('messages.unlimited_orders') }}</span>
                                @else
                                <span class="form-check-label text-dark"> {{ $store?->store_sub_update_application?->package?->max_order }} {{
                                    addon_published_status('Rental') ? translate('messages.trips') :  translate('messages.Orders') }} <small>({{ $store?->store_sub_update_application?->max_order }} {{ translate('left') }}) </small> </span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @endif
                                <span class="form-check-label text-dark">{{ translate('Kitchen Display') }}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="form-check-label text-dark">{{ translate('Unlimited Menu Items') }}</span>
                                @else
                                <span class="form-check-label text-dark"> {{ $store?->store_sub_update_application?->max_product }} {{ translate('Menu Items') }} <small>({{ $store?->store_sub_update_application?->max_product  - $store->items_count > 0 ? $store?->store_sub_update_application?->max_product  - $store->items_count : 0 }} {{ translate('left') }}) </small></span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                @endif
                                <span class="form-check-label text-dark">{{ translate('Menu Templates') }}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <img src="{{asset('/public/assets/admin/img/subscription-plan/check-1.png')}}" alt="">
                                @endif
                                <span class="form-check-label text-dark">{{ translate('Chat Support') }}</span>
                            </div>
                        </div>

                    </div>

                    @if($store?->store_sub_update_application?->package?->menuTemplates && $store?->store_sub_update_application?->package?->menuTemplates->count() > 0)
                    <div class="mt-3">
                        <h6 class="text--title mb-2">{{ translate('Included_Menu_Templates') }}</h6>
                        <div class="d-flex flex-wrap gap-2">
                            @foreach($store->store_sub_update_application->package->menuTemplates as $template)
                            <span class="badge badge-soft-info py-2 px-3">{{ $template->title }}</span>
                            @endforeach
                        </div>
                    </div>
                    @endif
                                ...
                                <div class="__plan-item hover {{ $store?->store_sub_update_application?->package_id == $package->id  && $store->store_business_model != 'commission'  ? 'active' : ''}}">
                                    <div class="inner-div">
                                        ...
                                        <ul class="info">
                                            ...
                                            @if ($package->pos)
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ translate('QR Menu') }} </span>
                                            </li>
                                            @endif
                                            @if ($package->mobile_app)
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ translate('Kitchen Display') }} </span>
                                            </li>
                                            @endif
                                            @if ($package->chat)
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ translate('Chat Support') }} </span>
                                            </li>
                                            @endif
                                            @if ($package->review)
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ translate('Menu Templates') }} </span>
                                            </li>
                                            @endif
                                            @if ($package->self_delivery)
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ translate('Online Ordering') }} </span>
                                            </li>
                                            @endif
                                            ...
                                            @else
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ $package->max_order }} {{ translate('Orders') }} </span>
                                            </li>
                                            @endif
                                            @if ($package->max_product == 'unlimited')
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ translate('Unlimited Menu Items') }} </span>
                                            </li>
                                            @else
                                            <li>
                                                <i class="tio-checkmark-circle"></i> <span>  {{ $package->max_product }} {{ translate('Menu Items') }} </span>
                                            </li>
                                            @endif
                                        ...
                                        <div class="text-center">
                                            ...
                                            <button data-id="{{ $package->id }}"  data-url="{{route('admin.settings.subscription.subscriptionackage.packageView',[$package->id,$store->id ])}}"
