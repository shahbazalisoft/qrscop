                    ...
                    @foreach(['all' => 'All', 'pending' => 'Pending', 'confirmed' => 'Confirmed', 'preparing' => 'Preparing', 'completed' => 'Completed', 'cancelled' => 'Cancelled'] as $key => $label)
                        <li class="nav-item">
                            <a class="nav-link {{ $status == $key ? 'active' : '' }}"
                               href="{{ route('admin.menu-order.list', $key) }}">
        ...
        <div class="card">
            ...
            <div class="card-header py-2 border-0">
                <div class="search--button-wrapper justify-content-end">
                    <form class="search-form min--260" method="GET" action="{{ route('admin.menu-order.list', $status) }}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn--container justify-content-center align-items-center gap-2">
                                            <form id="status-form-{{ $order->id }}" action="{{ route('admin.menu-order.status-update', $order->id) }}" method="POST">
                                                @csrf
                                                @method('PUT')
...
<script>
    ...
    $('.view-order-btn').on('click', function () {
        ...
        $.get("{{ url('admin/menu-order/quick-view') }}/" + orderId, function (data) {
