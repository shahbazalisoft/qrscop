    ...
    <div class="content container-fluid">
        ...
        <div class="js-nav-scroller hs-nav-scroller-horizontal mb-4">
            <ul class="nav nav-tabs border-0 nav--tabs nav--pills">
                ...
                <li class="nav-item">
                    <a href="#" class="nav-link active">{{ translate('Referral_Transactions') }}</a>
                </li>
            </ul>
        </div>
        ...
        <div class="card">
            <div class="card-header flex-wrap py-2 border-0">
                <div class="d-flex align-items-center gap-2 mb-2">
                    <h4 class="mb-0">{{ translate('Referral_History') }}</h4>
                    <span class="badge badge-soft-dark rounded-circle">{{ $referral_trans->total() }}</span>
                </div>
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-borderless middle-align __txt-14px">
                        <thead class="thead-light white--space-false">
                            ...
                            <th class="border-top px-4 border-bottom"><div class="text-title">{{ translate('Apply_Vendor') }}</div></th>
                            <th class="border-top px-4 border-bottom">{{ translate('referral_code') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('days') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('note') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('date') }}</th>
                            <th class="border-top px-4 border-bottom">{{ translate('status') }}</th>

                        </thead>
                        <tbody>
                            @foreach ($referral_trans as $k=> $transaction)

                            <tr>
                                <td class="px-4 text-center">{{ $k + $referral_trans->firstItem() }}</td>
                                <td class="px-4">
                                    <div class="pl-4">{{ $transaction->applyStore->name ?? 'N/A' }}</div>
                                </td>

                                <td class="px-4">
                                    <div class="text-title">{{ $transaction->apply_referral_code }}</div>
                                </td>

                                <td class="px-4">
                                    <div class="text-title">{{ $transaction->days }}</div>
                                </td>
                                <td class="px-4">
                                    <div class="text-title">{{ $transaction->note }}</div>
                                </td>
                                <td class="px-4">
                                    <div class="text-title">{{ \App\CentralLogics\Helpers::date_format($transaction->created_at) }}</div>
                                </td>
                                <td class="px-4">
                ...
                @if(count($referral_trans) !== 0)
                <hr>
                @endif
                <div class="page-area">
                    {!! $referral_trans->withQueryString()->links() !!}
                </div>
                @if(count($referral_trans) === 0)
