...
class ItemController extends Controller
{
    ...
    public function  getBrandList(Request $request){
        ...
        $data =  Brand::active()->where('name', 'like', '%'.$request->q.'%')->limit(10)->get();
    ...
    public function suggest(Request $request)
    {
        ...
        $items = Item::where('name', 'like', "%{$search}%")
            ->with('tags')
            ->limit(10)
            ->get(['id', 'name', 'description', 'category_id', 'category_ids', 'price', 'discount', 'discount_type', 'veg', 'available_time_starts', 'available_time_ends', 'food_variations', 'add_ons', 'maximum_cart_quantity', 'stock']);
        ...
        $result = $items->map(function ($item) {
            return [
                ...
                'category_ids' => is_string($item->category_ids) ? json_decode($item->category_ids, true) : $item->category_ids,
                'price' => $item->price,
                ...
                'food_variations' => is_string($item->food_variations) ? json_decode($item->food_variations, true) : $item->food_variations,
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'name.required' => translate('messages.item_default_name_required'),
            'description.required' => translate('messages.item_default_description_required'),
        ...
        // Handle gallery-picked images — store reference only, no file copy
        if ($request->has('gallery_images')) {
            foreach ($request->gallery_images as $galleryPath) {
                if (Storage::disk('local')->exists($galleryPath)) {
                    $images[] = ['img' => basename($galleryPath), 'storage' => Helpers::getDisk()];
                }
            }
        }
        ...
        // Handle thumbnail: file upload > gallery pick > keep existing
        if ($request->has('image')) {
            ...
            $food->image = basename($request->gallery_thumbnail);
        } else {
            $food->image = $newFileNamethumb ?? null;
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // } else {
            // Handle thumbnail: file upload > gallery pick > keep existing
            if ($request->has('image')) {
                $p->image = Helpers::update('product/', $p->image, 'png', $request->file('image'));
            } elseif ($request->has('gallery_thumbnail')) {
                // Store reference only — no file copy
                $p->image = basename($request->gallery_thumbnail);
            }
            ...
            if ($request->has('gallery_images')) {
                foreach ($request->gallery_images as $galleryPath) {
                    if (Storage::disk('local')->exists($galleryPath)) {
                        array_push($images, ['img' => basename($galleryPath), 'storage' => Helpers::getDisk()]);
                    }
                }
            }
    ...
    {
        $module_type = 'food';//Helpers::get_store_data()->module->module_type;
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if ($request->button == 'import') {
            ...
            try {
                foreach ($collections as $key => $collection) {
                    if ( $collection['Name'] === "" || $collection['MenuId'] === "" || $collection['Price'] === "" || $collection['Discount'] === "" || $collection['DiscountType'] === "") {
                    ...
                    if (!empty($collection['Images'])) {
                        $files = explode(',', $collection['Images']);
                    ...
                    array_push($data, [
                        ...
                        'images' => $images,//$collection['Images'] ?? json_encode([]),
                        'category_id' => $collection['MenuId'],//$collection['SubCategoryId'] ? $collection['SubCategoryId'] : $collection['MenuId'],
                        'category_ids' => json_encode([['id' => $collection['MenuId'], 'position' => 0]]),
                        ...
                        'available_time_starts' => '00:00:00',//$collection['AvailableTimeStarts'] ?? '00:00:00',
                        'available_time_ends' => '23:59:59',//$collection['AvailableTimeEnds'] ?? '23:59:59',
                        'variations' => $module_type == 'food' ? json_encode([]) : $collection['Variations'] ?? json_encode([]),
                        'food_variations' => !empty($collection['Variations']) ? $collection['Variations'] : json_encode([]),//$module_type == 'food' ? $collection['Variations'] ?? json_encode([]) : json_encode([]),
                        'add_ons' => json_encode([]),//$collection['AddOns'] ? ($collection['AddOns'] == "" ? json_encode([]) : $collection['AddOns']) : json_encode([]),
                        'attributes' => json_encode([]),//$collection['Attributes'] ? ($collection['Attributes'] == "" ? json_encode([]) : $collection['Attributes']) : json_encode([]),
                        'store_id' => Helpers::get_store_id(),
                        // 'module_id' => Helpers::get_store_data()->module_id,
                        'choice_options' => json_encode([]),//$module_type == 'food' ? json_encode([]) : $collection['ChoiceOptions'] ?? json_encode([]),
                        'status' => $collection['Status'] == 'active' ? 1 : 0,
                        'veg' => $collection['Veg'] == 'yes' ? 1 : 0,
                        'recommended' => 0,//$collection['Recommended'] == 'yes' ? 1 : 0,
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
        ...
        try {
            foreach ($collections as $key => $collection) {
                if ($collection['Id'] === "" || $collection['Name'] === "" || $collection['MenuId'] === "" || $collection['Price'] === "" || $collection['Discount'] === "" || $collection['DiscountType'] === "") {
                    Toastr::error(translate('messages.please_fill_all_required_fields'));
                    return back();
                }
                if (isset($collection['Price']) && ($collection['Price'] < 0)) {
                ...
                array_push($data, [
                    ...
                    'images' => $images,//$collection['Images'] ?? json_encode([]),
                    'category_id' => $collection['MenuId'],//$collection['SubCategoryId'] ? $collection['SubCategoryId'] : $collection['MenuId'],
                    'category_ids' => json_encode([['id' => $collection['MenuId'], 'position' => 0]]),//json_encode([['id' => $collection['MenuId'], 'position' => 0], ['id' => $collection['SubCategoryId'], 'position' => 1]]),
                    // 'unit_id' => is_int($collection['UnitId']) ? $collection['UnitId'] : null,
                    // 'stock' => is_numeric($collection['Stock']) ? abs($collection['Stock']) : 0,
                    'price' => $collection['Price'],
                    'discount' => $collection['Discount'],
                    ...
                    'available_time_starts' => '00:00:00',//$collection['AvailableTimeStarts'] ?? '00:00:00',
                    'available_time_ends' => '23:59:59',//$collection['AvailableTimeEnds'] ?? '23:59:59',
                    'variations' => json_encode([]),//!empty($collection['Variations']) ? $collection['Variations'] : json_encode([]),$module_type == 'food' ? json_encode([]) : $collection['Variations'] ?? json_encode([]),
                    'food_variations' => !empty($collection['Variations']) ? $collection['Variations'] : json_encode([]),//$module_type == 'food' ? $collection['Variations'] ?? json_encode([]) : json_encode([]),
                    'add_ons' => json_encode([]),//$collection['AddOns'] ? ($collection['AddOns'] == "" ? json_encode([]) : $collection['AddOns']) : json_encode([]),
                    'attributes' => json_encode([]),//$collection['Attributes'] ? ($collection['Attributes'] == "" ? json_encode([]) : $collection['Attributes']) : json_encode([]),
                    'store_id' => Helpers::get_store_id(),
    ...
    public function bulk_export_data(Request $request)
    {
        ...
        $products = Item::when($request['type'] == 'date_wise', function ($query) use ($request) {
            ...
            ->get();
        return (new FastExcel(ProductLogic::format_export_items(Helpers::Export_generator($products), 'food')))->download('Items.xlsx');
    }
