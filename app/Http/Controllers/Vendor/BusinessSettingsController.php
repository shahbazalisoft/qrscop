...
class BusinessSettingsController extends Controller
{
    ...
    public function updateStoreMetaData(Request $request, $storeId)
    {
        ...
        //     'image.image' => translate('image_must_be_image'),
        //     'image.mimes' => translate('image_format_jpg_png'),
        //     'image.max' => translate('image_max_size_2mb'),
        // ]);
        // Handle image upload
        if ($request->hasFile('banner_popup')) {
            $banner_popup = Helpers::update('store/',$store->banner_popup ?? null,'png',$request->file('banner_popup'));
        } else {
            $banner_popup = $store->banner_popup ?? null;
        }
        
        $existingData = $store->text_banner_popup ?? [];

        // Handle image upload
        if ($request->hasFile('image')) {
            $image = Helpers::update('store/',$existingData['image'] ?? null,'png',$request->file('image'));
        } else {
        ...
        // Save (❌ removed duplicate find)
        $store->text_banner_popup = $bannerData;
        $store->banner_popup = $banner_popup;
        $store->save();
    ...
    public function menu_template_cusomize()
    {
        $store = Helpers::get_store_data();
        
        return view('vendor-views.business-settings.menu-template-customize', compact('store'));
    }

    public function menu_template_cusomize_update($id, Request $request)
    {
        ...
        $store->delivery_charg = $request->delivery_charg??0;
        // $store->delivery_charge = $store->self_delivery_system?$request->delivery_charge??0: $store->delivery_charge;
        $store->banner_popup_type = $request->banner_popup_type??0;
        $store->order_type = $request->order_type??1;
        $store->alternative_logo = $request->has('image') ? Helpers::upload('store/', 'png', $request->file('image')) : $store->alternative_logo;
        $store->restaurant_secondary_name = $request->restaurant_secondary_name ?? $store->restaurant_secondary_name;
        $store->tracking_order_mobile_no = $request->tracking_order_mobile_no;
        $store->restaurant_title = $request->restaurant_title;
        $store->menu_buttom_home = $request->menu_buttom_home;
        $store->menu_buttom_special = $request->menu_buttom_special;
        $store->menu_buttom_cart = $request->menu_buttom_cart;
        $store->menu_buttom_reorder = $request->menu_buttom_reorder;
        $store->menu_buttom_orders = $request->menu_buttom_orders;
        ...
        // $conf->minimum_stock_for_warning = $request->minimum_stock_for_warning ?? 0;
        // $conf->save();
        Toastr::success(translate('messages.menu_settings_updated'));
        return back();
    }
}
