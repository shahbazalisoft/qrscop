...
class SubscriptionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $package_sell_count = SubscriptionPackage::when($request?->module != 1, function ($query) {
                $query->where('module_type', 'all');
            })
            ->when($request?->module == 1, function ($query) {
                $query->where('module_type', 'rental');
    ...
    public function create(Request $request)
    {
        ...
        return view('admin-views.subscription.package.create', compact('language', 'module', 'menuTemplates'));
    }
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'package_name.required' => translate('default_package_name_is_required'),
        ]);
        ...
        $package->module_type = 'all';
        $package->save();

        if ($request->menu_template_ids) {
            $package->menuTemplates()->sync($request->menu_template_ids);
        }

        AdminNotification::send(
            'New subscription package "' . $package->package_name . '" created',
            'subscription_created',
            'Package price: ' . $package->price . ', validity: ' . $package->validity . ' days',
            route('admin.settings.subscription.subscriptionackage.show', $package->id)
        );

        Toastr::success(translate('messages.Package_successfully_Added'));
        return redirect()->route('admin.settings.subscription.subscriptionackage.index');
    }
    ...
    {
        $subscriptionackage->load('menuTemplates');
    ...
    {
        $subscriptionackage->load('translations', 'menuTemplates')->withoutGlobalScope('translate');
        $language = getWebConfig('language');
        $defaultLang = str_replace('_', '-', app()->getLocale());
        $menuTemplates = MenuTemplate::where('status', 1)->get();
        return view('admin-views.subscription.package.edit', compact('language', 'defaultLang', 'subscriptionackage', 'menuTemplates'));
    }

    public function update(SubscriptionPackage $subscriptionackage, Request $request)
    {
        $request->validate([
            'package_name' => 'required|max:191|unique:subscription_packages,package_name,' . $subscriptionackage->id,
        ...
        $subscriptionackage->menuTemplates()->sync($request->menu_template_ids ?? []);

        AdminNotification::send(
            'Subscription package "' . $subscriptionackage->package_name . '" updated',
            'subscription_updated',
            'Package price: ' . $subscriptionackage->price . ', validity: ' . $subscriptionackage->validity . ' days',
            route('admin.settings.subscription.subscriptionackage.show', $subscriptionackage->id)
        );
        ...
        return redirect()->route('admin.settings.subscription.subscriptionackage.show', $subscriptionackage->id);
    }
    ...
    private function packageOverview($subscriptionackage, $type = 'all')
    {
        ...
        $totalSubscribersData = $subscriptionackage->subscribers()
            ...
            ->selectRaw(
                'COUNT(DISTINCT store_id) AS total_subscribers,
    ...
    public function qrPaymentRequests(Request $request)
    {
        $status = $request->status ?? 'pending';
        $requests = QrPaymentRequest::with(['store', 'package'])
        ...
        $qr_payment_image = BusinessSetting::where('key', 'qr_payment_image')->first()?->value;
        $qr_payment_details = BusinessSetting::where('key', 'qr_payment_details')->first()?->value;

        return view('admin-views.subscription.qr-payment-requests', compact('requests', 'status', 'qr_payment_image', 'qr_payment_details'));
    }

    public function qrPaymentRequestAction(Request $request, $id)
    {
        $request->validate([
        ...
        $qrRequest = QrPaymentRequest::findOrFail($id);
        $qrRequest->status = $request->status;
        $qrRequest->admin_note = $request->admin_note;
        $qrRequest->save();

        if ($request->status == 'approved') {
            $store = Store::find($qrRequest->store_id);
            $package = SubscriptionPackage::find($qrRequest->package_id);
            if ($store && $package) {
            ...
            Toastr::success(translate('Payment approved and subscription activated successfully.'));
        } else {
            Toastr::info(translate('Payment request rejected.'));
        }

        return back();
    }

    public function qrPaymentSettingsUpdate(Request $request)
    {
        $request->validate([
            'qr_payment_image' => 'nullable|image|mimes:jpg,jpeg,png,webp|max:5120',
            'qr_payment_details' => 'nullable|string|max:1000',
        ]);

        if ($request->hasFile('qr_payment_image')) {
            $imageName = Helpers::upload('qr_payment/', 'png', $request->file('qr_payment_image'));
        ...
        BusinessSetting::updateOrCreate(
            ['key' => 'qr_payment_details'],
            ['value' => $request->qr_payment_details]
        );

        Toastr::success(translate('QR payment settings updated successfully.'));
        return back();
    }
}
