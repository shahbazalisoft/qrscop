...
class BusinessSettingsController extends Controller
{
    ...
    public function website_appearance()
    {
        if (!Helpers::module_permission_check('settings')) {
            Toastr::error(translate('messages.access_denied'));
            return back();
        ...
        $colors = Helpers::get_business_settings('website_colors');
        if (is_string($colors)) {
            $colors = json_decode($colors, true);
        }
        $colors = $colors ?? [
            'primary_color' => '#10847E',
            ...
            'text_muted' => '#888888',
            'border_color' => '#2a2a2a',
            'btn_hover_color' => '#0c6b66',
        ];

        return view('admin-views.business-settings.website-appearance', compact('colors'));
    }

    public function website_appearance_update(Request $request)
    {
        if (env('APP_MODE') == 'demo') {
            Toastr::info(translate('messages.update_option_is_disable_for_demo'));
            return back();
        ...
        $colors = [
            'primary_color' => $request->primary_color ?? '#10847E',
            'secondary_color' => $request->secondary_color ?? '#1a1a2e',
            'dark_bg' => $request->dark_bg ?? '#0d0d0d',
            'light_bg' => $request->light_bg ?? '#f8f9fa',
            'text_light' => $request->text_light ?? '#ffffff',
            'text_dark' => $request->text_dark ?? '#333333',
            'text_muted' => $request->text_muted ?? '#888888',
            'border_color' => $request->border_color ?? '#2a2a2a',
            'btn_hover_color' => $request->btn_hover_color ?? '#0c6b66',
        ];

        Helpers::businessUpdateOrInsert(['key' => 'website_colors'], [
            'value' => json_encode($colors),
        ]);

        Toastr::success(translate('messages.website_appearance_updated_successfully'));
        return back();
    }
    ...
    public function business_setup(Request $request)
    {
        ...
        Helpers::businessUpdateOrInsert(['key' => 'home_banner_popup'], [
            'value' => $request['home_banner_popup'],
        ]);

        if ($request->hasFile('home_banner_popup_image')) {
            $imageName = time() . '_home_banner_popup.' . $request->home_banner_popup_image->extension();
            $request->home_banner_popup_image->move(public_path('assets/landing/image'), $imageName);
            Helpers::businessUpdateOrInsert(['key' => 'home_banner_popup_image'], [
                'value' => $imageName,
            ]);
        }
