...
class ItemController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Handle gallery-picked images
        if ($request->has('gallery_images')) {
            foreach ($request->gallery_images as $galleryPath) {
                if (Storage::disk('local')->exists($galleryPath)) {
                    $ext = pathinfo($galleryPath, PATHINFO_EXTENSION) ?: 'png';
                    $newFileName = Carbon::now()->toDateString() . "-" . uniqid() . "." . $ext;
                    $disk = Helpers::getDisk();
                    Storage::disk($disk)->put("product/{$newFileName}", Storage::disk('local')->get($galleryPath));
                    $images[] = ['img' => $newFileName, 'storage' => $disk];
                }
            }
        }
        ...
        // Handle thumbnail: file upload > gallery pick > gallery copy fallback
        if ($request->has('image')) {
            ...
            if (Storage::disk('local')->exists($galleryThumbPath)) {
                ...
                $newThumbName = Carbon::now()->toDateString() . "-" . uniqid() . "." . $ext;
                $disk = Helpers::getDisk();
                Storage::disk($disk)->put("product/{$newThumbName}", Storage::disk('local')->get($galleryThumbPath));
                $item->image = $newThumbName;
            }
        } else {
            $item->image = $newFileNamethumb ?? null;
        }
    ...
    {
        $product = Item::withoutGlobalScope(StoreScope::class)->withoutGlobalScope('translate')->with('store', 'category')->findOrFail($id);
            ...
            if ($request->has('gallery_images')) {
                foreach ($request->gallery_images as $galleryPath) {
                    if (Storage::disk('local')->exists($galleryPath)) {
                        ...
                        $disk = Helpers::getDisk();
                        Storage::disk($disk)->put("product/{$newFileName}", Storage::disk('local')->get($galleryPath));
                        array_push($images, ['img' => $newFileName, 'storage' => $disk]);
                    }
                }
            }
        }
    ...
    public function list(Request $request)
    {
        ...
        return view('admin-views.product.list', compact('items', 'store', 'category', 'type', 'sub_category'));
    }
    ...
    {
        $module_type = 'food';//Config::get('module.current_module_type');
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if ($request->button == 'import') {
            ...
            try {
                foreach ($collections as $collection) {
                    if ($collection['Name'] === "" || $collection['MenuId'] === ""|| $collection['Price'] === "" || $collection['StoreId'] === ""|| $collection['Discount'] === "" || $collection['DiscountType'] === "") {
                    ...
                    array_push($data, [
                        ...
                        'images' => $images,
                        'category_id' => $collection['MenuId'],//$collection['SubCategoryId'] ? $collection['SubCategoryId'] : $collection['CategoryId'],
                        ...
                        'food_variations' => !empty($collection['Variations']) ? $collection['Variations'] : json_encode([]),//!empty($collection['Variations']) ?? json_encode([]),//$module_type == 'food' ? $collection['Variations'] ?? json_encode([]) : json_encode([]),
        ...
        try {
            foreach ($collections as $collection) {
                ...
                if($request->upload_type === 'update' && $collection['Id'] === ""){
                    Toastr::error(translate('messages.please_fill_all_id_required_fields'));
                    return back();
                }
                if (isset($collection['Price']) && ($collection['Price'] < 0)) {
