...
class DashboardController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // --- Date filter helper ---
        $dateFilter = function ($query, $column = 'created_at') use ($statisticsType) {
            return match ($statisticsType) {
                ...
                default => $query, // overall — no date filter
            };
        };
        ...
        $totalMenuOrders = \App\Models\MenuOrder::count();
        $totalCustomers = User::count();

        $newStores = $dateFilter(Store::query())->count() - ($statisticsType === 'overall' ? 0 : 0);
        $newItems = $dateFilter(Item::query())->count();
        $newMenuOrders = $dateFilter(\App\Models\MenuOrder::query())->count();
        $newCustomers = $dateFilter(User::query())->count();

        // For "newly added" label: show period-specific count
        if ($statisticsType === 'overall') {
            // Show last 30 days as "new"
            $newStores = Store::where('created_at', '>=', now()->subDays(30))->count();
            $newItems = Item::where('created_at', '>=', now()->subDays(30))->count();
            $newMenuOrders = \App\Models\MenuOrder::where('created_at', '>=', now()->subDays(30))->count();
            $newCustomers = User::where('created_at', '>=', now()->subDays(30))->count();
        }

        // --- Menu Order status counts ---
        $menuOrderPending = \App\Models\MenuOrder::where('status', 'pending')->count();
        $menuOrderConfirmed = \App\Models\MenuOrder::where('status', 'confirmed')->count();
        $menuOrderPreparing = \App\Models\MenuOrder::where('status', 'preparing')->count();
        $menuOrderCompleted = \App\Models\MenuOrder::where('status', 'completed')->count();
        $menuOrderCancelled = \App\Models\MenuOrder::where('status', 'cancelled')->count();

        // --- Revenue chart data ---
        $commissionOverview = $request->input('commission_overview', 'this_year');
        $months = [
        ...
        if ($commissionOverview === 'this_week') {
            $weekStart = now()->startOfWeek();
            for ($i = 0; $i < 7; $i++) {
                $date = $weekStart->copy()->addDays($i);
                $menuRevenue[$i] = \App\Models\MenuOrder::whereDate('created_at', $date->format('Y-m-d'))
                    ->where('status', '!=', 'cancelled')->sum('total');
            }
            $label = $days;
        } elseif ($commissionOverview === 'this_month') {
            $daysInMonth = now()->daysInMonth;
            for ($i = 1; $i <= $daysInMonth; $i++) {
                $menuRevenue[$i] = \App\Models\MenuOrder::whereDay('created_at', $i)
            ...
            // this_year
            for ($i = 1; $i <= 12; $i++) {
                $menuRevenue[$i] = \App\Models\MenuOrder::whereMonth('created_at', $i)
                    ->whereYear('created_at', now()->year)
                    ->where('status', '!=', 'cancelled')->sum('total');
            }
            $label = $months;
        ...
        $totalRevenue = array_sum($menuRevenue);

        // --- Top stores by total_order ---
        $topStores = Store::where('total_order', '>', 0)
            ->orderByDesc('total_order')
            ->take(6)->get();

        // --- Newly joined stores (last 30 days) ---
        $newJoinedStores = Store::where('created_at', '>=', now()->subDays(30))
            ->latest()->take(6)->get();

        // --- Top selling items (by order_count) ---
        $topSellingItems = Item::where('order_count', '>', 0)
            ->orderByDesc('order_count')
            ->take(6)->get();

        // --- Top stores by visits ---
        $topVisitedStores = Store::where('total_visits', '>', 0)
            ->orderByDesc('total_visits')
            ->take(6)->get();

        // --- Top stores by menu orders ---
        $topOrderStores = Store::withCount('menu_orders')
            ->having('menu_orders_count', '>', 0)
            ->orderByDesc('menu_orders_count')
            ->take(10)->get();

        return view("admin-views.dashboard", compact(
            'statisticsType', 'commissionOverview',
            'totalStores', 'totalItems', 'totalMenuOrders', 'totalCustomers',
            'newStores', 'newItems', 'newMenuOrders', 'newCustomers',
            'menuOrderPending', 'menuOrderConfirmed', 'menuOrderPreparing',
            'menuOrderCompleted', 'menuOrderCancelled',
            'menuRevenue', 'label', 'totalRevenue',
            'topStores', 'topVisitedStores', 'newJoinedStores',
            'topSellingItems', 'topOrderStores'
        ));
    }
