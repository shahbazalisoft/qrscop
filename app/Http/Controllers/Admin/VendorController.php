...
class VendorController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'logo' => 'required|image|max:2048|mimes:jpg,jpeg,png,gif,webp',//.IMAGE_FORMAT_FOR_VALIDATION,
            'cover_photo' => 'nullable|image|max:2048|mimes:jpg,jpeg,png,gif,webp',//.IMAGE_FORMAT_FOR_VALIDATION,

        ], [
            'f_name.required' => translate('messages.first_name_is_required'),
            'name.required'=>translate('name_is_required'),
            'address.required'=>translate('address_is_required'),
        ]);
      ...
      if ($validator->fails()) {
        ...
        try {
            ...
            AdminNotification::send(
                'New store "' . $store->name . '" has been created',
                'store_created',
                'A new store was registered by ' . $vendor->f_name . ' ' . $vendor->l_name,
    ...
    public function update(Request $request, Store $store)
    {
        $validator = Validator::make($request->all(), [
            ...
            'logo' => 'nullable|image|max:2048|mimes:jpg,jpeg,png,gif,webp',//.IMAGE_FORMAT_FOR_VALIDATION,
            'cover_photo' => 'nullable|image|max:2048|mimes:jpg,jpeg,png,gif,webp',//.IMAGE_FORMAT_FOR_VALIDATION,
        ], [
            'f_name.required' => translate('messages.first_name_is_required'),
            'name.required'=>translate('name_is_required'),
    ...
    public function view(Request $request,$store_id, $tab=null, $sub_tab='cash')
    {
        ...
        {
            $foods = Category::where('store_id', $store->id)
                ->where('position',0)
                ->when(isset($key) , function($q) use($key){
                    $q->where(function ($q) use ($key) {
                        foreach ($key as $value) {
            ...
            return view('admin-views.vendor.view.menu', compact('store','foods'));
        }
