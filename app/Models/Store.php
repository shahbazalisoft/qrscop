...
class Store extends Model
{
    ...
    protected $casts = [
        ...
        'distance' => 'float',
        'menu_colors' => 'array',
        'text_banner_popup' => 'array',
    ];

    /**
     * @var string[]
     */
    protected $appends = ['gst_status','gst_code','logo_full_url','cover_photo_full_url','text_banner_image_full_url','tin_certificate_image_full_url'];
    ...
    public function getBannerPopupFullUrlAttribute(){
        $value = $this->banner_popup;
        if (count($this->storage) > 0) {
            foreach ($this->storage as $storage) {
                if ($storage['key'] == 'banner_popup') {
                    return Helpers::get_full_url('store',$value,$storage['value']);
                }
            }
        }
    ...
    public function getTextBannerImageFullUrlAttribute(){
        $data = $this->text_banner_popup ?? [];
        ...
        if (!empty($this->storage) && is_array($this->storage)) {
            foreach ($this->storage as $storage) {
                if (($storage['key'] ?? null) === 'image') {
                    return Helpers::get_full_url('store', $value, $storage['value'] ?? 'public');
                }
            }
        }
    ...
    public function getQrFoodImage1FullUrlAttribute(){
        $value = $this->qr_food_image_1;
        if(!$value) return null;
        if (count($this->storage) > 0) {
            foreach ($this->storage as $storage) {
                if ($storage['key'] == 'qr_food_image_1') {
                    return Helpers::get_full_url('store/qr-food',$value,$storage['value']);
                }
            }
        }
        return Helpers::get_full_url('store/qr-food',$value,'public');
    }
    public function getQrFoodImage2FullUrlAttribute(){
        $value = $this->qr_food_image_2;
        if(!$value) return null;
        if (count($this->storage) > 0) {
            foreach ($this->storage as $storage) {
                if ($storage['key'] == 'qr_food_image_2') {
                    return Helpers::get_full_url('store/qr-food',$value,$storage['value']);
                }
            }
        }
    ...
    public function getQrFoodImage3FullUrlAttribute(){
        $value = $this->qr_food_image_3;
        if(!$value) return null;
        if (count($this->storage) > 0) {
            foreach ($this->storage as $storage) {
                if ($storage['key'] == 'qr_food_image_3') {
                    return Helpers::get_full_url('store/qr-food',$value,$storage['value']);
                }
            }
        }
    ...
    public function getQrFoodImage4FullUrlAttribute(){
        $value = $this->qr_food_image_4;
        if(!$value) return null;
        if (count($this->storage) > 0) {
            foreach ($this->storage as $storage) {
                if ($storage['key'] == 'qr_food_image_4') {
                    return Helpers::get_full_url('store/qr-food',$value,$storage['value']);
                }
            }
        }
    ...
    public function menu_orders(): HasMany
    {
    ...
    protected static function boot(): void
    {
        ...
        static::created(function ($store) {
            ...
            $store->restaurant_title    = "Fresh Food · Great Taste";
            $store->menu_buttom_home    = "Home";
            $store->menu_buttom_special = "Specail";
            $store->menu_buttom_cart    = "Cart";
            $store->menu_buttom_orders  = "Orders";
            $store->referral_code       = 'QRS-' . strtoupper(substr(md5(uniqid(mt_rand(), true)), 0, 6));
            $store->save();
        });
